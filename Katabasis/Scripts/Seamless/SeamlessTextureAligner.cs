...
public static class SeamlessTextureAligner 
{
    ...
    public static int GetSpriteNum(Entity entity)
    {
        ...
        bool TexturesAreNOTEqual(TileData tile)
        {
            ...
            var type = entity.GetComponentData<CurrentTileComponent>().objectType;
            List<Entity> entities = new List<Entity>();
            switch (type)
            {
                case ObjectType.Solid: entities.Add(tile.SolidLayer);
                    break;
                case ObjectType.Drop: entities.AddRange(tile.DropLayer);
                    break;
                case ObjectType.Floor: entities.Add(tile.FloorLayer);
                    
                    break;
                case ObjectType.Liquid:entities.Add(tile.LiquidLayer);
                    break;
                default:
            ...
            {
                if (item != Entity.Null && item.GetComponentData<IDComponent>().ID == entity.GetComponentData<IDComponent>().ID) return false;
            }
            return true;
        }
    }

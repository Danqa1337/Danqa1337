...
public class CelularAutomaton : BaseMethodsClass
{
    ...
    public static int[,] GenerateMap(int size, float wallPercent, int iterationsCount, int scale, float pers, float lac, float borderFactor)
    {
        ...
        return ProcessMap(wallPercent, iterationsCount, borderFactor, initialMap);

    }

    public static int[,] ProcessMap(float wallPercent, int iterationsCount, float borderFactor, float[,] initialMap)
    {
        ...
        for (int x = 0; x < sizeX; x++)
        {
            for (int y = 0; y < sizeY; y++)
            {
                ...
                if ((y == 0 || x == 0 || y == sizeY - 1 || x == sizeX - 1)
                    || Chance((Mathf.Clamp01(initialMap[x, y]) 
